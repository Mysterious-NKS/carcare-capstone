    ...
    public function save()
{
    ...
    // Ensure appointment belongs to logged-in staff (if column exists)
    ...
    // Handle photos -> store JSON into **photos** column (matches your DB)
    $uploaded = [];
    ...
    if ($uploaded && $this->colExists($pdo, 'service_records', 'photos')) {
        // merge with existing photos if present
        $st = $pdo->prepare("SELECT photos FROM service_records WHERE appointment_id=?");
        $st->execute([$appointment_id]);
        $row = $st->fetch(PDO::FETCH_ASSOC);
        $existing = [];
        if ($row && !empty($row['photos'])) {
            $parsed = json_decode($row['photos'], true);
            if (is_array($parsed)) $existing = $parsed;
        }
        $fields['photos'] = json_encode(array_values(array_unique(array_merge($existing, $uploaded))));
    }
    ...
    if ($rid) {
        if (!empty($fields)) {
            $set = [];
            $vals = [];
            foreach ($fields as $col => $val) { $set[] = "$col=?"; $vals[] = $val; }
            $vals[] = $appointment_id;
